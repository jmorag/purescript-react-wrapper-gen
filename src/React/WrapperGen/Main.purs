...
propType { "type": Just { name } } = case name of
  "bool" -> PBool
  "func" -> PFunc
  "number" -> PNumber
  "string" -> PString
...
propType { flowType: Just { name } } = case name of
  "string" -> PString
  "number" -> PNumber
  "boolean" -> PBool
  -- "any"
...
mapStrInitial f s = case CU.uncons s of
  Just { head: c, tail: s' } -> (CU.singleton <<< f) c <> s'
  Nothing -> ""
...
type TranslateResults
  = { extern :: String, js :: String, props :: String, name :: String }
...
getType default baseFname fname name prefix info =
  let
    types = map (\(Tuple s a) -> Tuple s $ propType a) $ toUnfoldable info.props
    ...
    js =
      "exports." <> camelName <> "Class = require('" <> requireName <> "')"
        <> (if default then ".default" else "")
        <> ";\n"

    props =
      "foreign import data " <> name <> "Option :: Type\n"
        <> "newtype "
        <> nameProps
        <> " = "
        <> nameProps
        <> " Foreign\n"
        <> toLowerInitial nameProps
        <> " :: Options "
        <> name
        <> "Option -> "
        <> nameProps
  ...
  showFieldOption (Tuple pname ty) =
    optName <> " :: Option " <> name <> "Option (" <> showType ty <> ")\n"
      ...
      <> "\n"
    where
    optName = mungeName $ toLowerInitial pname

  mungeName n
    | n == "type" || n == "inputProps" = n <> "'"
    | otherwise = n

  showField (Tuple pname (PUnknown s)) = "-- " <> pname <> " :: {" <> s <> "}"

  showField (Tuple pname ty) =
    (if pname /= toLowerInitial pname then "-- " else "")
      <> pname
      <> " :: "
      <> showType ty

getOutputFiles :: FilePath -> String -> String -> String -> Boolean -> Array FilePath -> Effect  Unit
getOutputFiles baseFname moduleName prefix outDir default files = do
  ...
  let
    infos' :: Array TranslateResults
    infos' = A.mapMaybe identity infos
...
"""
        <> "import "
        <> moduleName
        <> " (EventHandlerOpt, UnknownType, Node)\n"
        <> extern
        <> props
  ...
  void
    $ traverse
        ...
        infos'
  where
  getOutputType :: FilePath -> Effect (Maybe TranslateResults)
  getOutputType fname =
    catchException
      ( \e -> do
...
main = do
  let
    ...
    setup =
      usage "$0 --base base/dir --module ModuleName --prefix prefix/dir -o outputdir files..."
        <> defaultHelp
